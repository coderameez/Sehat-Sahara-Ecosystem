...
type BookingStep = 'recipient' | 'address' | 'date' | 'time' | 'reason' | 'records' | 'review' | 'confirmed';

export const HomeVisitBooking: React.FC = () => {
  ...
  const handleNext = () => {
    if (step === 'recipient') setStep('address');
    else if (step === 'address') setStep('date');
  ...
  const handleBack = () => {
    ...
    else if (step === 'date') setStep('address');
    else if (step === 'address') setStep('recipient');
    else goBack();
  };
  ...
  const handleConfirm = () => {
    const newBooking: Booking = {
      ...
      fee: provider.fees.direct_home_visit || 0,
      paymentPolicy: 'pay_at_clinic', // Home visit payment
    };
      ...
      <MobileAppShell>
        <div className="flex flex-col h-full bg-[#F8FAFC] p-6 items-center justify-center">
          ...
          <p className="text-[15px] text-slate-600 text-center mb-8 leading-relaxed max-w-[280px]">
            Your home visit by {provider.name} is confirmed for {selectedDate || dates[0]} at {selectedTime || times[0]}. The doctor will arrive at the provided address.
          </p>
  ...
  const renderStepIndicator = () => {
    const stepOrder = ['recipient', 'address', 'date', 'time', 'reason', 'records', 'review'];
    return (
      ...
      <div className="flex flex-col h-full bg-[#F8FAFC]">
        ...
        <header className="px-4 py-4 bg-white flex items-center border-b border-slate-200 shrink-0 sticky top-0 z-10">
          ...
          <div className="ml-2 flex-1 min-w-0">
            ...
            <p className="text-[13px] text-slate-500 truncate">{provider.name} (Home Visit)</p>
          </div>
        </header>
        ...
        <div className="app-scroll flex-1 px-5 py-6 pb-28">
          ...
          {step === 'address' && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-300">
              <h2 className="text-xl font-bold text-slate-900 mb-6">Visit Address</h2>
              <Input
                ...
                <div className="flex flex-col">
                  ...
                  <div className="flex p-4 border-b border-slate-100">
                    <div className="flex-1">
                      ...
                      <p className="text-[15px] font-semibold text-slate-900">Home Visit</p>
                    </div>
                  </div>
                  ...
                  <div className="flex p-4 border-b border-slate-100">
                    <div className="flex-1">
                      <p className="text-[12px] font-bold text-slate-400 uppercase tracking-wider mb-1">Visit Address</p>
                      <p className="text-[15px] font-semibold text-slate-900">{address || 'No address provided'}</p>
                    </div>
                    <button onClick={() => setStep('address')} className="text-[#1B7F4C] text-sm font-bold p-2 h-fit">Edit</button>
                  </div>
                  ...
                  <div className="p-4 bg-slate-50 flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-[13px] text-slate-600 font-medium">Payment: Cash on visit</p>
                    </div>
                    <p className="text-xl font-black text-slate-900">PKR {provider.fees.direct_home_visit || 0}</p>
                  </div>
